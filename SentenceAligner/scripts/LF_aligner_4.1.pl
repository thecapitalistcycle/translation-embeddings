...
use IO::Handle;	# for autoflush on the log file
...
# consider adding a) b) c) d) e) f) to dictionary

# implement the realign mode if null.dic is used

# GUI for batch mode

# In HTML conversion, s/<\/td>/\n/g;

# no-hunalign merge mode
...
# if ($filetype) { if ($filetype eq "t" or $filetype eq "h" or $filetype eq "p") {$filetype_prompt = "n"} elsif (!$version =~ /sajat/) {abort("Wrong filetype specified in command line argument; only t, h and p are supported at line " . __LINE__)}  };
...
if ($gui) {
	...
	print "Defaults: lang_1_iso_def: $lang_1_iso_def; lang_2_iso_def: $lang_2_iso_def; tmx_langcode_1_def: $tmx_langcode_1_def; tmx_langcode_2_def: $tmx_langcode_2_def; creationid_def: $creationid_def; tool: $tool; version: $version; GUIlang: $guilang; GUIfile: $scriptpath/scripts/GUIstrings_${guilang}.txt";
} else {
...
unless (@l) {
	...
	for (my $i = 0; $i < $no; $i++) {
		...
		do {
			...
			if ($i == 4) {$l[$i] or $l[$i] = "nl";}
			if ($i == 5) {$l[$i] or $l[$i] = "pt";}
			if ($i == 6) {$l[$i] or $l[$i] = "el";}
...
if ($filetype eq "c") {
	...
	# generate CELEX from natural number
		...
		# print LOG "\n$celexyear: $celexyear, serial: $celexserial, celex: $celex\n";
...
##########################
# NEW EUR_LEX URL FORMAT #
##########################
#do
# new url: http://eur-lex.europa.eu/legal-content/FR/TXT/HTML/?uri=CELEX:32007L0023
	...
	for (my $i = 0; $i < $no; $i++) {
		...
		$url[$i] = "http://eur-lex.europa.eu/legal-content/$l[$i]/TXT/HTML/?uri=CELEX:$celex"; # 
		print LOG "\n$l[$i] URL: $url[$i]"
...
# PREPARE FILES BEFORE RUNNING THE STRIPPER
	...
	#do if filetype is c, loop through the file and strip everything before <p class="doc-ti (assuming this string occurs in the file)
