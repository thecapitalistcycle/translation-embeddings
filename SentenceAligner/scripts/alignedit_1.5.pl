...
# Control-Key-V - paste at cursor, or at end, not replace whole cell

# setup option to do column operations without setheight; - or running setheight on the next 100 rows only
...
$menu_help->command(
	...
	-command => sub {
		my $shortcuts = $mw->Dialog(-title => 'Keyboard shortcut cheat sheet', 
				   -text => "Ctrl-O:		Open file\nCtrl-S:		Save file\nF1:		Merge\nF2:		Split\nF3:		Shift up\nF4:		Shift down\nF5:		Delete row\nShift-F5:		Delete cell\nF6:		Move nearest non-empty cell up\nShift-F6:		Move nearest non-empty cell down\nF7:		Split upwards\nF8:		Split and shift down\nF11:		Search\nF12:		Jump to next empty segment\nShift-left/right:	Move the cursor by one character\nCtrl-left/right:	move the cursor by one word\nCtrl-Alt-left/right:	move the cursor by one sentence\nCtrl-up/down:	move the cursor to the beginning/end of the active cell\nCtrl-N:		Copy cell\nCtrl-X:		Cut cell\nCtrl-M:		Paste cell\nCtrl-Z:		Undo (you can only undo one operation)",
...
# SCROLLING
# $mw->bind('<MouseWheel>'=>[sub{$t->yview('scroll', -($_[1]/120),'units')} ,Ev('D')]); # mouse wheel scrolling code from https://groups.google.com/forum/?fromgroups=#!topic/comp.lang.perl.tk/TpQqAEeqN1k
...
# $mw->bind('<MouseWheel>'=>[sub{$t->yview('scroll', -($_[1]/120),'units')} ,Ev('D')]);
...
$mw->bind('<F7>' => [sub {clone; splitup;}]); # only available through this shortcut
$mw->bind('<F8>' => [sub {clone; split_shiftdown;}]); # only available through this shortcut
...
my $clipboard;
$mw->bind('<Control-Key-n>' => [sub {$clipboard = $t->get('active');}]);	# copy!

$mw->bind('<Control-Key-b>' => [sub {
	clone;
	my $actidx = $t->index('active');
	my ($actrow, $actcol) = split(",", $actidx);
	$clipboard = $t->get('active');
...
# paste copied cell
$mw->bind('<Control-Key-m>' => [sub {
	clone;	# make this one reversible
	
	my $actidx = $t->index('active');
	my ($actrow, $actcol) = split(",", $actidx);
	my $actcont = $arrayVar->{"$actrow,$actcol"};
	...
	$aftercurs =~ s/^\s//;
	# print "\n\nbefore: >$beforecurs<\n\nafter: >$aftercurs<\n\n";
	# $arrayVar->{"$actrow,$actcol"} .= " $clipboard";
	$arrayVar->{"$actrow,$actcol"} = "$beforecurs $clipboard $aftercurs";
	$arrayVar->{"$actrow,$actcol"} =~ s/^\s//;
	$arrayVar->{"$actrow,$actcol"} =~ s/\s$//;
	...
	# print "\n paste done, $actrow $actcol modified to $arrayVar->{\"$actrow,$actcol\"}\n";
}]);
...
sub mergerow {
	...
	# }
		...
		for my $col (0 .. $cols - 2) {
		# print "\nMerging row $actrow col $col\n";
		my $actcont = $arrayVar->{"$actrow,$col"};
		my $belowactcont = $arrayVar->{"$belowactrow,$col"};
		$arrayVar->{"$belowactrow,$col"} = "$actcont $belowactcont" if $actcont; # add contents of above cell
		$arrayVar->{"$belowactrow,$col"} =~  s/^\s//;		# just in case the active cell was empty
		$arrayVar->{"$belowactrow,$col"} =~  s/\s$//;		# just in case the active cell was empty
		$arrayVar->{"$actrow,$col"} = ""; 	# empty the active cell
	}
...
sub addcolumns {
	my $in_type = [ ['Txt', ['.txt'] ], ['All Files', '*'] ];
			...
			my $file = $mw ->getOpenFile(	#do add my here and test
										-title => "Please choose the input file",
										-filetypes => $in_type,
										-multiple => 0,
									);
...
sub addcolumns_autoalign {
	...
	# system ("\"$scriptpath\\scripts\\hunalign\\hunalign\" -text \"$scriptpath\\scripts\\hunalign\\data\\en-en.dic\" \"$scriptpath/other_tools/lang1.txt\" \"$file_mod\" > \"$scriptpath/other_tools/aligned.txt\""); # or error("\nCan't run hunalign: $!"); # always raises an error
	
	# print "\nran hunalign, created $scriptpath/other_tools/aligned.txt\n";
	# <STDIN>;
	...
	unless ( open (ALIGNED_MOD, ">:encoding(UTF-8)", "$scriptpath/other_tools/aligned_mod.txt") ) { error("Can't open file for writing: $!");return};
	
	# print "\nremoved confidence values\n";
	# <STDIN>;
	...
	# print "\nstretching done: $scriptpath/other_tools/aligned_mod.txt\n";
	# <STDIN>;
	...
	# print "\nmerging done: $scriptpath/other_tools/aligned_mod.txt\n";
	# <STDIN>;
	unless ( rename ("$scriptpath/other_tools/aligned_mod.txt", "$scriptpath/other_tools/aligned.txt") ) {error("\nCan't rename file: $!");return};
	
	
	
	# Import aligned file back into editing table
